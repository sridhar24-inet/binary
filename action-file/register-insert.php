...
if ($count_array < 3) {
    ...
    if ($result) {
        ...
        if($row_s11['left_leg']=='')
        {
            $update1 = "UPDATE people SET left_leg='$profile_id' WHERE left_leg='' AND profile_id='$placement_id'";
            $result1 = mysqli_query($con, $update1);
            if($result1)
            {
                echo "<script>window.location.replace('../index.php?success=true');</script>";
            }
            
        }
        elseif($row_s11['right_leg']=='')
        {
            $update = "UPDATE people SET right_leg='$profile_id' WHERE right_leg='' AND profile_id='$placement_id'";
    ...
    if ($result) {
        ...
        if($row_s1['left_leg']=='')
        {
            $update = "UPDATE people SET left_leg='$profile_id' WHERE left_leg='' AND profile_id='$placement_id'";
